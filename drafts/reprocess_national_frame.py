...
# ---- 2b. Load population weights (2011 Census, KS101EW) ----
ks101 = pd.read_csv(KS101_PATH)
ks101.columns = ks101.columns.str.strip()
pop_candidates = [c for c in ks101.columns
                  if 'all' in c.lower() and 'usual' in c.lower()]
pop_col = pop_candidates[0] if pop_candidates else ks101.columns[1]
code_col = [c for c in ks101.columns
            if 'geography' in c.lower() or 'code' in c.lower()][0]
lsoa_pop = (ks101[[code_col, pop_col]]
            .rename(columns={code_col: 'lsoa11cd', pop_col: 'pop'}))
