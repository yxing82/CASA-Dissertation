...
periods into a single 3-column alluvial diagram:
  ...
  Column 2  →  Flow-based restructuring typology (CFI Churn + Net Cascade)
...
# Typology from Methodology §3.9:
#   Step 1 — CFI Churn median splits Stable vs active restructuring
#   Step 2 — Among high-churn MSOAs, Net Cascade quartiles determine
#            the restructuring regime (ascent / circulation / displacement)
#
# Thresholds are computed from the 2011 Census period (baseline flows).

churn_med = df['CFI_Churn_11'].median()
high_churn_mask = df['CFI_Churn_11'] >= churn_med
nc_q25 = df.loc[high_churn_mask, 'Net_Cascade_11'].quantile(0.25)
nc_q75 = df.loc[high_churn_mask, 'Net_Cascade_11'].quantile(0.75)

print(f'Thresholds:  CFI Churn median = {churn_med:.0f}')
print(f'             Net Cascade Q25  = {nc_q25:.0f}  (high-churn only)')
print(f'             Net Cascade Q75  = {nc_q75:.0f}  (high-churn only)')

def classify(row):
    if row['CFI_Churn_11'] < churn_med:
        return 'Stable'
    nc = row['Net_Cascade_11']
    if nc >= nc_q75:
...
# Category descriptions (from Methodology §3.9)
cat_descriptions = {
    'Ascent-Dominated': 'High cascade churn with strongly positive Net Cascade (upper quartile). These MSOAs exhibit restructuring dynamics consistent with neighbourhood ascent and socio-economic upgrading — stronger upward-directed inflows relative to downward outflows.',
    'Cascading Circulation': 'High cascade churn with relatively balanced Net Cascade (interquartile range). These MSOAs experience substantial hierarchical mobility turnover without a dominant directional imbalance, suggesting continuous circulation and cascading movement through the neighbourhood.',
    'Displacement-Dominated': 'High cascade churn with strongly negative Net Cascade (lower quartile). These MSOAs exhibit intensified outward displacement pressure within the deprivation hierarchy — stronger downward-directed outflows relative to upward inflows.',
    'Stable': 'Below-median CFI Churn, indicating relatively limited hierarchical restructuring regardless of directional flow balance. The neighbourhood position within the deprivation hierarchy remained largely unchanged.',
}
...
.bar-label{width:46px;color:var(--t3);font-weight:500;font-size:10px}
...
<div class="app">
  ...
  <p class="subtitle">
    ...
    through a <strong>flow-based restructuring typology</strong> to their <strong>2019 IMD decile</strong>.
    Classification uses CFI Churn (median threshold) to identify active restructuring, then Net Cascade
    quartiles to distinguish ascent-dominated, displacement-dominated, and cascading circulation regimes.
  </p>
  <div class="controls">
  ...
  <div id="viewBreakdown" style="display:none">
    <p style="font-size:12.5px;color:var(--t3);margin:0 0 16px;line-height:1.5">
      ...
      and outflow to poorer areas (lower-ranked), across both census periods. Inspired by the
    ...
    <div class="insight-box">
      ...
      <p class="insight-text">
        ...
        than leaving to poorer ones, consistent with ascent-dominated restructuring. Deciles 6–10 show the
        mirror: net negative cascade, as outflows to poorer areas exceed inflows from wealthier, indicating
        displacement-dominated dynamics. The pattern persists across both census periods, though cascade
        volumes decreased slightly between 2011 and 2021. The symmetry pivot around Decile 5 is the
        clearest fingerprint of the cascading displacement mechanism.
      </p>
    </div>
  </div>
</div>
...
<div class="source">
  ...
  Classification: CFI Churn (median threshold) × Net Cascade direction (quartile thresholds among high-churn MSOAs)
</div>
...
// ── CONSTANTS ────────────────────────────────────────────────────────
const CAT_COLORS = {
  'Ascent-Dominated':'#2563eb',
  'Cascading Circulation':'#7c3aed',
  'Displacement-Dominated':'#dc2626',
  'Stable':'#64748b'
};
const DECILE_COLORS = [
...
// ── ALLUVIAL LAYOUT ──────────────────────────────────────────────────
// D10 (least deprived) at TOP, D1 (most deprived) at BOTTOM
// Matches Yee (2021) figure and standard IMD convention.
function computeLayout(){
  ...
  // Reversed order: D10 at top → D1 at bottom
  ...
  // Left→Mid bands (sorted by draw order for proper stacking)
  const lmBands = [];
  const lOff = {}, mLOff = {};
...
function drawAlluvial(){
  ...
  html += `<text x="${mCx}" y="24" text-anchor="middle" font-size="13" font-weight="700" fill="#333" font-family="DM Sans">Restructuring Regime</text>`;
  html += `<text x="${mCx}" y="40" text-anchor="middle" font-size="10" fill="#999" font-family="DM Sans">(CFI Churn + Net Cascade)</text>`;
  ...
  // Deprivation scale labels — D10 at top, D1 at bottom
  ...
  D.cats.forEach(c=>{
    ...
    if(n.h>28){
      ...
      if(words.length > 1 && n.h > 36){
        ...
        html += `<text x="${n.x+n.w/2}" y="${n.y+n.h/2-4}" text-anchor="middle"
          dominant-baseline="central" font-size="10" font-weight="700" fill="#fff"
          font-family="DM Sans" style="pointer-events:none">${n.cat}</text>`;
  ...
  // Right nodes (D10 at top → D1 at bottom)
  L.drawOrder.forEach(d=>{
...
function drawBreakdown(){
  ...
  let html = '';
  // D10 (least deprived) first, D1 (most deprived) last
      ...
      <div class="decile-head">
        <div style="display:flex;align-items:center">
          ...
          <div class="decile-info">
            <div class="decile-name">IMD Decile ${ds.d}</div>
